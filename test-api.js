...
// Import API_BASE_URL from CommonJS config file
const { API_BASE_URL } = require("./config.cjs");

console.log("Testing API connection to:", API_BASE_URL);

// Test the basic connection endpoint
axios
  .get(`${API_BASE_URL}/api/test-connection/`)
  .then((response) => {
  ...
  .catch((error) => {
    ...
    if (error.response) {
      ...
      console.error(
        "No response received from server. Server might be down or network issue."
      );
    } else {
